...
const MultiSelectValue = React.forwardRef<React.ElementRef<typeof Primitive.div>, MultiSelectValueProps>(
	({ className, placeholder, maxDisplay, maxItemLength, ...props }, forwardRef) => {
		...
		return (
			<TooltipProvider delayDuration={300}>
				<div
					className={cn('flex flex-1 overflow-x-hidden flex-wrap items-center gap-1.5', className)}
					{...props}
					ref={forwardRef}>
					...
					{renderRemain ? (
						<span className="text-muted-foreground text-xs leading-4 py-.5">+{renderRemain}</span>
					) : null}
				</div>
			</TooltipProvider>
		)
...
>(({ className, ...props }, ref) => {
	return <CommandList ref={ref} className={cn('py-1 px-0 max-h-[unset]', className)} {...props} />
})
...
>(({ className, children, ...props }, ref) => {
	...
	const fragmentRef = useRef<DocumentFragment>(null)
	...
	return (
		<PopoverPrimitive.Portal forceMount>
			<PopoverPrimitive.Content
				...
				{...props}>
				<Command
					className={cn('px-1 max-h-96 w-full min-w-[var(--radix-select-trigger-width)]', className)}
